...
- **Organic over-yield after a clean R1 (2026-04-18, cycle `brainstorm-register`, observation).** In the `creative_first_opener` 5-round organic bundle, the panel yielded to the user for R2/R3/R4 after a strong Creative R1 that asked an open-ended question. The role-player returned meaningful substance in R2 (*"feedback system, not just constraint. That's the thing I'm missing."*), but the orchestrator did not re-engage a specialist until R5. Not a regression of this cycle's edits — Creative R1 was correct, and the yield pattern is likely from prior orchestrator guidance. Signal to watch: when brainstorm register lands well, the orchestrator may over-read "give the user space" and fail to re-engage the specialist when the user has volunteered new substance. If this pattern recurs in future brainstorm-register transcripts, the close path is an orchestrator prompt edit along the lines of *"if the user answered a specialist's question with new substance, re-engage the specialist — don't yield"*. Do not fix preemptively.
