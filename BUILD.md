...
**Status:** IN PROGRESS — 7.1 Marketer + Finance + Realist + Creative-v2 (voice+cases, cycle `creative-rep` 2026-04-19) · 7.2 instrument · 7.3 Marketer-layer + Finance + Realist + Creative · 7.4 · 7.5 · 7.6 · 7.7 all SHIPPED. **Next coherent block: 7.1/7.2/7.3 replication to the remaining 5 specialists** per the revised 4-cycle plan in [docs/handoffs/2026-04-19-specialist-evaluation-plan.md](docs/handoffs/2026-04-19-specialist-evaluation-plan.md) — Cycle 2 Copywriter · Cycle 3 CX · Cycle 4 light-touch trio (Ops/Legal/Accountant bundled).
...
- **Organic over-yield after a clean R1 (2026-04-18 observation; 2026-04-19 diagnostic, cycle `over-yield-diag`; still open after cycle `creative-rep`).** Initial observation: `creative_first_opener` 5-round organic bundle yielded R2–R4 after a clean Creative R1. Diagnostic cycle 2026-04-19 ran creative_first_opener 3× organic + 5 off-arc organic controls (pleasantries_first, lagged_answerer, jerk_rusher, therapist_cautious, vague_thought). **Strict pre-registered rule (R2+R3 both yield after R1 open question, ≥2/3) did NOT reproduce — 1/3.** BUT a different shape reproduced 4/4: **R3 yields in all 4 creative-first runs and R4 yields in all 4** (R2 is stochastic — yields 2/4, engages 2/4; R3+R4 are deterministic). 0/5 off-arc runs had any yield round — pleasantries/lagged/jerk/therapist/vague all engaged every round with multi-specialist depth. **Cycle `creative-rep` (2026-04-19) update:** Creative v2 voice rewrite did NOT close this flag, as expected — it targets voice/cases, not orchestrator routing. The creative_first_opener organic run in cycle `creative-rep` showed the same pattern: Creative fires R1 + R2 with strong commit-with-exclude turns, then panel silence R3–R6. With Creative v2 now in place, the scope for the next cycle's close attempt is cleaner: add 1–2 concept-first personas (product concept, brand name), pre-register the R3+R4 rule properly, and decide whether the orchestrator prompt needs a "continue examining the concept" nudge after R2 Creative or whether the yield is actually correct (the owner has named what they wanted to explore and the panel has nothing useful to add without new signal). Bundles: `test/results/creative_first_opener_20260419_02510*_over-yield-diag/` (3 runs) and 5 off-arc bundles same cycle; `test/results/creative_first_opener_20260419_095455_creative-rep/` (cycle `creative-rep` confirmation). Do not fix preemptively.
- **Creative firing rate across batch (2026-04-19, cycle `creative-rep`, hidden-assumptions lens).** Creative v2 fires in 2/15 persona runs across the cycle's Stage 1 + Stage 2 partial + Stage 3 (creative_first_opener, ai_consultant — both reference-quality commit-with-exclude). Zero Creative turns across 13 other personas covering pro-services, local-services, restaurant, fitness, ecommerce, forced-constraint, and legal-primary shapes. The load-bearing assumption: **13% fire rate is narrow-by-design (Creative is a counterweight specialist, not a default), not a routing gap.** Revealed state is ambiguous: personas like `glory_days_vintage` (Savannah vintage shop with loyal-aesthetic Instagram following, named after mom Gloria) and `poza_salon` have surface-level angle-gap potential but went entirely to numerical specialists. No evidence these personas' recommendations suffered for Creative's absence — the recs hit 6/6 grader — but hand-read did not stress-test this specifically. **Close path next cycle:** spot-check 2 angle-gap-potential batch recs (e.g., poza_salon, glory_days_vintage) for whether they commit to prioritization or default to category language where a Creative counterweight would have helped. If they commit, Creative's absence is correct and the widened-routing thesis is already scoped right. If they go generic, the orchestrator's Creative-as-R3-counterweight trigger needs loosening. Not a cycle-blocking flag.

### 7.1 Specialist voice rewrite

**Status:** Marketer v2/v3 SHIPPED (2026-04-17 / 2026-04-18) · **Finance v2 SHIPPED (2026-04-18)** · **Realist v2 SHIPPED (2026-04-18)** · **Creative v2 SHIPPED (2026-04-19)**. Other 6 NOT yet rewritten.
...
- [ ] Add divergence rule to all specialist prompts (rollout with 7.1/7.3 replication — Marketer has it in v3; Finance has it in v2; Realist has it in v2; Creative has it in v2; 6 remaining).
...
- [ ] Add evidence-bound rule to all specialist prompts (Marketer + Finance + Realist + Creative have it; 6 remaining).
...
**Status:** Marketer layer SHIPPED (2026-04-18) · **Finance case library SHIPPED (2026-04-18)** · **Realist case library SHIPPED (2026-04-18)** · **Creative case library SHIPPED (2026-04-19)**. Other 6 case libraries NOT yet created.
...
- [x] **Creative case library shipped (2026-04-19):** 14 cases in [lib/agents/cases/creative.json](lib/agents/cases/creative.json) indexed by `business_type_category` (concept_first ×4, professional_services ×3, local_services ×2, restaurant_food ×2, fitness_wellness ×2, ecommerce_dtc ×1). Each case exhibits the commitment + exclusion shape — the one angle to build on AND the nearby truth to rule out. Added **`concept_first`** as a new `BusinessTypeCategory` ([lib/knowledge/loader.ts](lib/knowledge/loader.ts)); case-loader now prefers concept_first-tagged cases when `inferBusinessType` returns null, typical of concept-first openers ([lib/agents/case-loader.ts](lib/agents/case-loader.ts) — Option F from cycle plan). Minimal ripple: `CATEGORY_KEYWORDS['concept_first'] = []` (inference-by-absence), `CATEGORY_CHANNELS['concept_first'] = []` (never reached because `inferBusinessType` still returns null for these cases — the new entry only satisfies `Record<BusinessTypeCategory, ...>` exhaustiveness).
...
- [ ] **Specialist replication — the remaining block.** 6 × (case library JSON + validation against naturally-triggering personas in the full batch). Pair with each specialist's 7.1 voice rewrite and 7.2 rule rollout; pick one specialist at a time per the 4-cycle plan.
