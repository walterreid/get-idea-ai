...
**Status:** IN PROGRESS — 7.1 Marketer + Finance + Realist · 7.2 instrument · 7.3 Marketer-layer + Finance + Realist · 7.4 · 7.5 · 7.6 · 7.7 all SHIPPED. **Next coherent block: 7.1/7.2/7.3 replication to the remaining 7 specialists** (Realist cycle `realist-rep` closed 2026-04-18; brainstorm-register cycle queued after).
...
- **Brainstorm-register routing gap (2026-04-18, manual /chat session).** Creative-first openers (game idea, product concept) routed to Realist for 3 turns before Creative was summoned. Realist's questions were valid in isolation but created a transactional delay before the creative exploration the user came for. Not a Realist prompt bug — a routing calibration. Scheduled cycle: after current specialist replication block. See [docs/manual_chat_2026-04-18_game_brainstorm.md](docs/manual_chat_2026-04-18_game_brainstorm.md). Paired with: Creative token-cap raising (220→350 for brainstorm turns) and orchestrator false-interrupt-attribution fix.
- **Creative token-cap truncation (2026-04-18, manual /chat session).** Creative's 220-token cap causes mid-sentence truncation in brainstorm turns (observed 3× in one session). Orchestrator then misattributes the truncation to a user interrupt (a product-surface correctness bug). Both fixes belong in the brainstorm-register cycle. See [docs/manual_chat_2026-04-18_game_brainstorm.md](docs/manual_chat_2026-04-18_game_brainstorm.md).
...
- [ ] **Other 7 specialists — next block.** Pattern from Marketer/Finance/Realist replication: voice rewrite + cases + §7.2 rules + description tightening, one per cycle. Per manual /chat session (2026-04-18), the **brainstorm-register cycle** comes next (orchestrator routing edit + Creative description + Creative token-cap raise) before continuing the specialist roster. Operations and Accountant under-fire; CX has been holding its own. Pick one per cycle for focused iteration.
